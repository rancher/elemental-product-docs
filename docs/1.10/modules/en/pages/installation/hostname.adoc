...
The hostname set in the `machineName` field is expected to be in a template form, in order to be uniquely generated for each registering node, using the xref:label-templates/label-templates-random.adoc[Random] variables from the xref:label-templates/label-templates.adoc[Label Templates] feature.
...
This way {elemental-product-name} will use the current hostname as the `MachineInventory` name during the registration phase, which will be later set as the static hostname of the host during the provisioning phase. Below is a registration example with the hostname and `MachineInventory` name set on the hostname received by the DHCP server.
