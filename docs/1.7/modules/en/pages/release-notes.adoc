...
| v1.7.5
| https://github.com/rancher/elemental-operator/releases/tag/v1.7.5[Link]

| https://github.com/rancher/elemental-toolkit/[{elemental-product-name} Toolkit]
| v2.2.9
| https://github.com/rancher/elemental-toolkit/releases/tag/v2.2.9[Link]
...
In order to install this release of the Elemental Operator check the xref:installation/installation.adoc[installation documentation]. For already existing deployments use the following Helm commands to upgrade:
...
Any version prior to v1.9.x is not compatible with Rancher v2.14.x and above.
...
The SLE Micro OS images with versions v2.1.1 and v2.1.2 (released in the default xref:references/managedosversionchannel-reference.adoc[ManagedOSVersionChannel]) adopt predictable network interface names by default.
...
xref:references/managedosversionchannel-reference.adoc[ManagedOSVersionChannel]) do not allow ssh root access via password anymore. A workaround is to either configure ssh root access via an ssh key or add a new user to the system.
