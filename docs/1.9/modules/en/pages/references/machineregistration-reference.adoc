...
Template used to derive the hostname to be set to the node and as the name of the associated xref:references/machineinventory-reference.adoc[MachineInventory] kubernetes resource.

The value is interpolated using xref:label-templates/label-templates.adoc[Label Templates].

[NOTE]
====
If no `machineName` is specified, a default one in the form `m-$UUID` will be set.

See the xref:hostname.adoc#_customize_hostname[Customize Hostname section] for further details.
====
...
The label values are interpolated using xref:label-templates/label-templates.adoc[Label Templates].

These labels could be used to establish a selection criteria in xref:references/machineinventoryselectortemplate-reference.adoc[MachineInventorySelectorTemplate].
