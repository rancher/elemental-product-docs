...
----
The {product-name} Operator chart is distributed via an OCI registry: Helm correctly supports OCI based registries starting from the v3.8.0 release.
----
...
----
When upgrading from an elemental-operator release embedding the {product-name} CRDs (version < 1.2.4) the elemental-operator-crds chart installation will fail.
