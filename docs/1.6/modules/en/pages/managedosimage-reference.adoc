...
[CAUTION]
====
When using any SUSE® Rancher Prime OS Manager or xref:custom-images.adoc[SUSE® Rancher Prime OS Manager based image] you are expected to only edit the `env` key to optionally set the `FORCE`, `UPGRADE_RECOVERY`, or `UPGRADE_RECOVERY_ONLY` variables. +
