...
The `MachineRegistration` is needed to perform the deployment of the {product-name} OS on the target hosts. When booting up, each host registers to the {product-name} Operator which tracks the new host with a `MachineInventory` resource.
...
The `SeedImage` is required to generate the *seed image* (like a bootable ISO) that will boot and start the {product-name} provisioning on the target machines.
...
+
====
The `SeedImage` resource, which automates the creation of an {product-name} bootable image (the *seed image*), does not support Raspberry Pi ISOs yet (click [here](raspi-disk.md) for a guide to build a raw disk image).
...
Using quickstart files from {product-name} docs repo directly::
+
========
You can directly apply the quickstart example resource files from the [{product-name} docs repository](https://github.com/rancher/elemental-docs). 
...
This is the last step: you need an {product-name} seed image that includes the initial registration config, so it can be auto registered, installed and fully deployed as part of your cluster.
...
+
======
{product-name}'s support for Raspberry Pi is primarily for demonstration purposes at this point. Therefore the installation process is modelled similar to x86-64. You boot from a seed image (an USB stick in this case) and install to a storage medium (SD-card for Raspberry Pi). 
...
The written USB stick will have two partitions. `RPI_BOOT` contains the boot loader files and `COS_LIVE` the {product-name} files. Mount the `COS_LIVE` partition and write `initial-registration.yaml` as `livecd-cloud-config.yaml` to this partition. 
 ...
 ** ssh into the machine (user/pass: root/ros):
  *** running `journalctl -f -t elemental` shows you the progress of the registration (_elemental-register_) and the installation of {product-name} (_elemental install_).
