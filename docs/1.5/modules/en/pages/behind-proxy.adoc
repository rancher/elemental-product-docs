...
In a lot of enterprise environments, servers or VMs running on premises do not have direct Internet access. Instead, the connection to external services is done through a HTTP(S) proxy for security reasons. This tutorial shows you how to set up an {product-name} deployment in such an environment.
...
* SeedImage resource
* {product-name} cluster configuration
...
xref:architecture-components.adoc#_elemental_register_command_line_tool[{product-name}-register] is the first communication endpoint between the new host and Rancher Manager, this is the first place where proxy settings need to be set.
...
The process happens when you boot your {product-name} ISO for the first time, in order to configure the proxy settings you have to include a `cloud-init` definition in the ISO.
...
+
====
image:proxy-settings-cluster-ui.png[Add proxy settings for {product-name} cluster]
====
======
