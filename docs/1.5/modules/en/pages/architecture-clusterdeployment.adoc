...
The {product-name} Kubernetes cluster deployment involves the following steps:
...
* The {product-name} Operator pairs each generated xref:machineinventoryselector-reference.adoc[MachineInventorySelector] resource with an available xref:machineinventory-reference.adoc[MachineInventory] and installs the https://github.com/rancher/system-agent[rancher-system-agent] daemon on the host tracked by the xref:machineinventory-reference.adoc[MachineInventory].
