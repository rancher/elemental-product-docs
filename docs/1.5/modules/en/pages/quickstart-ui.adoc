= SUSE® Rancher Prime OS Manager the visual way
...
This quickstart will show you how to deploy the SUSE® Rancher Prime OS Manager plugin and operator into an existing Rancher Manager instance.

Once installed, you'll be able to provision a new SUSE® Rancher Prime OS Manager cluster based on RKE2 or K3s.
...
image::quickstart-ui-elemental-plugin-install.png[SUSE® Rancher Prime OS Manager plugin install]
...
[NOTE]
====
The following guide will show you how to install the operator through the SUSE® Rancher Prime OS Manager UI. But you can also install it directly from the Marketplace.
====

Click on the OS Management button in the navigation menu.

If the operator is not already installed, the elemental ui will let you deploy it by clicking on the `Install SUSE® Rancher Prime OS Manager Operator` button:
...
image::quickstart-ui-extension-operator-install-1.png[Install SUSE® Rancher Prime OS Manager operator screenshot 1]

In this screen, you can customize or use the default values, click on `Install` to continue:

image::quickstart-ui-extension-operator-install-2.png[Install SUSE® Rancher Prime OS Manager operator screenshot 2]

You should see ``elemental-operator-crds``and `elemental-operator` deployed in the `cattle-elemental-system` namespace:

image::quickstart-ui-extension-operator-install-3.png[Install SUSE® Rancher Prime OS Manager operator screenshot 3]
...
`device-selector`: The xref:machineregistration-reference.adoc#configelementalinstalldevice-selector[device-selector] field can be used to dynamically pick device during installation. The field contains a list of rules to select the device you want.
...
In opposite to ISO where it needs two devices (device with ISO and another disk where to install SUSE® Rancher Prime OS Manager), RAW image allows to boot from a single device and directly install the operating system in the device.
RAW image only contains a boot and a recovery partition and it boots first into recovery mode to install SUSE® Rancher Prime OS Manager (for information, the process is similar to the xref:reset.adoc#_reset_workflow[reset] one).
...
On the following screenshot, xref:hardwarelabels.adoc#_hardware_labels[`Hardware Labels`] are used as custom columns:
...
== Create your first SUSE® Rancher Prime OS Manager Cluster

Now let's use those `Machine Inventory` to create a cluster by clicking on `Create SUSE® Rancher Prime OS Manager Cluster` :

image::quickstart-ui-create-cluster-button.png[Create SUSE® Rancher Prime OS Manager Cluster button]

For your SUSE® Rancher Prime OS Manager cluster, you can either choose K3s or RKE2 for Kubernetes.

image::quickstart-ui-create-cluster-standard-screen-.png[SUSE® Rancher Prime OS Manager Cluster Creation Screen]
...
It lets you choose which `Machine Inventory` you want to use to create your SUSE® Rancher Prime OS Manager cluster using the previously defined `Machine Inventory Labels` :

image::quickstart-ui-create-cluster-machine-selector-template.png[Use Machine Inventory Selector Template]

As our three Machine Inventories contain the label `CPUVendor` with the key `AuthenticAMD`, the three machines will be used to create the SUSE® Rancher Prime OS Manager cluster.
