...
{product-name} uses the https://github.com/rancher/system-agent[Rancher System Agent], renamed to {product-name} System Agent, to initially bootstrap the node with a simple plan.
...
[TIP]
====
The local plans run only during the initial {product-name} bootstrap *before* Kubernetes is installed on the node.
====


== Types of Plans

The type of plans that {product-name} can use are:
...
== Adding local plans on {product-name}

You can add local plans to {product-name} as part of the `MachineRegistration` CRD, in the `cloud-config` section as follows:
