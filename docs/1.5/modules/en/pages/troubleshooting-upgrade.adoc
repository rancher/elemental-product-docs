...
spec:
  # Set to the new {product-name} version you would like to upgrade to or track the latest tag
...
== {product-name} Cluster side

Any {product-name} node correctly registered and part of the target cluster will fetch the bundle and start applying it. +
This operation is performed by the Rancher's `system-upgrade-controller` running on the {product-name} Cluster. +
...
*This is to be expected*, as {product-name} relies on the job to be ran again after the machine restarts, so that it can verify the new version was installed correctly. +
